...
function Connection(manager, req, socket, data, options){
  ...
  if( !checkVersion(this)) {
    ...
    socket.addListener('timeout', function () {
      ...
      manager.emit("timeout", connection);
    ...
    this.addListener("stateChange", function(state, laststate){
      ...
      if(state === 4){
        manager.attach(connection.id, connection);
        manager.emit("connection", connection);
        ...
        manager.detach(connection.id, function(){
          manager.emit("close", connection);
...
function checkVersion(client){
  var version = client._options.version.toLowerCase();

  return (version == "auto" || version == client.version);
};
...
var doHandshake = {
  ...
  draft75: function(){
    ...
    if(location){
      ...
      if(this._options.subprotocol && typeof this._options.subprotocol == "string") {
        res += "\r\nWebSocket-Protocol: "+this._options.subprotocol;
      }
  ...
  draft76: function(){
    ...
    if(location){
      ...
      if(this._options.subprotocol && typeof this._options.subprotocol == "string") {
        res += "\r\nSec-WebSocket-Protocol: "+this._options.subprotocol;
      }
